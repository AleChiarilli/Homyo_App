...
def create_contract_pro_cmr():
    ...
    print(body)
    contract = Contract(posted_by=user.id, pro_profile_id=pro_profile.id, cmr_profile_id=home.cmr_profile_id, comment=home_post.description, finishing_time=home_post.finishing_time, starting_time=home_post.starting_time, home_id=home.id, hourly_rate=pro_profile.hourly_rate, total_price=body["total_price"],home_post_id=body["home_post_id"])
...
def create_contract_cmr_pro():
    ...
    home = Home.query.filter_by(id=home_post.home_id).first()
    print(body)
    contract = Contract(posted_by=user.id, pro_profile_id=pro_profile.id, cmr_profile_id=cmr_profile.id, comment=body["comment"], finishing_time=body["finishing_time"], starting_time=body["starting_time"], home_id=home.id , hourly_rate=pro_profile.hourly_rate, total_price=body["total_price"],home_post_id=body["home_post_id"])
    db.session.add(contract)
    db.session.commit()
    skill_name = body.get("skill_name")
    skill = Skill.query.filter_by(name=skill_name).first() #se busca skill para añadir a profile skill
